...
# Clase Principal
class DlgDepositar(QDialog):
    def __init__(self, usuario, cuenta): # Constructor __init__
        super(DlgDepositar, self).__init__()
        loadUi('./uis/deposit.ui', self)
    ...
    def anotacion(self):
        ...
        if sMonto != "" and sMonto != "0":
            ...
            stMvto = (f"INSERT INTO movimientos(movimiento, fecha_mvto, monto, id_cuenta) VALUES (1, CURRENT_TIMESTAMP(), {monto}, {self.cuenta['id_cuenta']})")

            cursor.execute(st)
            cursor.execute(stMvto)
            mibanco.commit()

            self.lblMessage.setText(f"Transacción exitosa! Su saldo es ${nuevoSaldo:9,.2f}")
        else:
